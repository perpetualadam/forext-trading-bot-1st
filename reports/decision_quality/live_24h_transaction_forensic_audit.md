...
Old sample: 73 fills, 51 PositionClose, 34 of those in 46–90s (the known bug). New sample: 45 fills, 32 broker stops, **0 take-profits**, 10 PositionClose all winners held ≥15m (Postgres `exit_reason=profit_protection`), 2 TradeClose at the 11:41 restart. The 61-second bug does not recur after 09:40:11Z.
...
2026-09-21 11:41:00Z  TRADE_CLOSE AUD_USD 2181
2026-09-21 11:41:10Z  TRADE_CLOSE USD_JPY 2155
2026-09-21 11:41:55Z  current container start (broker-exit accounting in running image)
...
2026-09-21 21:21:47Z  AUD_USD 2434 STOP_LOSS — sample end
```
...
| BOT_PROFIT_PROTECTION_OLD_BUG | 33 | Old build, 46–90s PositionClose (incl. 1589/1597/1621). Postgres `profit_protection` where present |
| BOT_LOCAL_SLTP_OLD_BUG | 1 | 1629, 61s, Postgres `sl_tp` |
| BOT_PROFIT_PROTECTION_VALID | 10 | New-build PositionClose; PG `profit_protection`; holds 19–213 min; all winners; `source=OANDA_CURRENT` |
| OTHER_BOT_CLOSE | 17 | Old-build PositionClose **not** in the 46–90s cluster (longer PP and stale-mid `sl_tp`; plus 09:20–09:39 pre-restart cluster) |
| OTHER_BROKER_CLOSE | 2 | `TRADE_CLOSE` at 11:41:00Z / 11:41:10Z (TradeClose API, not bot PositionClose) |
| WEEKEND_FLATTEN | 0 | Friday-only; sample is Sun–Mon |
| MANUAL_CLOSE | 0 proven | The 2 TradeCloses are the only non-bot flatten candidates (**HYPOTHESIS**: restart-adjacent / UI) |
| UNKNOWN | 0 | Every close has an OANDA reason |
...
|---|---:|---:|---:|---:|---:|---:|---:|
| Old 46–90s PP bug | 33 | 1 | 27 | 5 | −0.0066 | −0.00020 | 61s |
| Old 61s local SL/TP (1629) | 1 | 0 | 0 | 1 | 0 | 0 | 61s |
| Old other PositionClose | 17 | 6 | 7 | 4 | −0.0031 | −0.00018 | ~20 min (mixed) |
| New legitimate PP | 10 | 10 | 0 | 0 | **+0.0088** | +0.00088 | 3386s |
| Weekend flatten | 0 | | | | | | |
| Manual | 0 proven | | | | | | |
| **All closeouts** | **61** | 17 | 34 | 10 | **−0.0009** | ≈0 | 61s |

Hold buckets (all 61 closeouts): 0–30s 0; 31–45s 1; **46–90s 34**; 91–120s 0; 2–5m 2; 5–15m 2; 15m+ 22.

**How much of the near-breakeven comes from the old bug? PROVEN:** the 34-row 46–90s cluster is −0.0066. Removing it, remaining closeouts are +0.0057. The headline −0.0009 is the old bug offsetting later valid PP (+0.0088). **CONFOUNDED_BY_OLD_BUG.**
...
| 6 | Some trades open/close around one minute | **SUPPORTED** old; **UNSUPPORTED** as current manage behaviour | 34× ~61s old PositionClose. After fix: no such bot closes. |
...
| 12 | 2 TradeCloses at 11:41Z | **INSUFFICIENT EVIDENCE** (not PositionClose) | MEDIUM |
